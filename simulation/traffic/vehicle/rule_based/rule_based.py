...
class RuleBased(Vehicle):
    ...
    def __init__(self, perception_size, speed_limit, acceleration, de_acceleration, length, type1):
        ...
        self.__two_second_rule_margin = 0.01

    @property
    def two_second_rule_margin(self):
        return self.__two_second_rule_margin

    @two_second_rule_margin.setter
    def two_second_rule_margin(self, two_second_rule_margin):
        self.__two_second_rule_margin = two_second_rule_margin

    def lane_change_assistant(self, side_lane_points, side_d_points, side_car_list):
        if len(side_car_list) == 0:
            return False

        _neigh_1, _neigh_2 = DrivingCalculations.get_neighbouring_points(side_lane_points, [self.x, self.y])
    ...
    def make_decision(self, grid, lane_points, d_points, right_lane_points, right_d_points, right_car_list,
        ...
        margin_point = self.speed_limit - (self.speed_limit * self.__two_second_rule_margin)

        if self.speed > self.speed_limit or two_sec_decision == "DeAccelerate":
            self._Vehicle__decision = "DeAccelerate"
            return "DeAccelerate"
